...
<div class="row">
                            <div class="col-xs-12">
                                ...
                                <div class="row">
                                    ...
                                    <?php echo $state; ?> GST Bills Pending List
                                        </div>
             ...
             <div class="col-sm-2"><b><a href="qut_gst_excel.php?user=<?php echo $tsname; ?>&state=<?php echo $state; ?>" class="btn btn-primary btn-xs">XL Download</a></b></div>
            	</div>
											...
											if(isset($_POST['bsearch'])){
											    ...
											    $y="select * from ".$request_amnt." where gsttype='With GST'  LIMIT $start_from,$results_per_page";
							                	}else { 
							                	    
							                	    if(($name=='admin') or ($name=='durgarao') or ($name==$state.'billing') or ($name=='accounts') ){
												 $y="select * from ".$request_amnt." where gsttype='With GST' LIMIT $start_from,$results_per_page";
							                	    }else{
							                	         $y="select * from ".$request_amnt." where gsttype='With GST' and user='$tsname' LIMIT $start_from,$results_per_page";
							                	    }
											...
											while($rs1=mysqli_fetch_array($t)){
												$q=$rs1['quet_num'];
												//$ramtn
												$r=mysqli_query($link,"select * from ".$qottable." where quet_num='$q'") or die(mysqli_error($link));
...
$currentDateTime = date('Y-m-d');
                                                   ...
                                                   <td class="hidden-480"><?php echo $rs1['ac_det'] ?></td>
                                                        ...
                                                        <td class="hidden-480">
                                                           <?php  if(($name=='admin') or ($name=='durgarao') or ($name=='accounts') ){ ?>
                                                            <a href="apfile_edit.php?id=<?php echo $rs1['id']; ?>&state=<?php echo $state; ?>">
                                                     ...
                                                     <?php   }
														...
														<td class="hidden-480">
														   <?php   if(($name=='admin') or ($name=='durgarao') or ($name=='accounts') ){ ?>
														    <a href="apfile_view.php?id=<?php echo $rs1['id']; ?>&state=<?php echo $state; ?>">
                                                        <img src="images/edit.gif"></a>
											...
											//$ssq=mysqli_query($link,"select * from ".$qottable." where status='Not Required' quet_num='$bsearch'");
...
echo "<li><a href='work2.php?state=$state&page=".($page-1)."' class='button'>Previous</a></li>"; 

echo "<li><a>".$page."</a></li>";

echo "<li><a href='work2.php?state=$state&page=".($page+1)."' class='button'>NEXT</a></li>";
echo "</ul>";
?>
