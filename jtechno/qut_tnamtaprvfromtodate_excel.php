...
if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts') or ($tsname=='knbilling') or ($tsname=='sumanthpotluri')){
	$y="select * from tnrequest_amnt where  confirm='Yes' and status='Amount Transferred' and date and date between '$fromdate' and '$todate'   order by id desc";    
}else{
    $y="select * from tnrequest_amnt where  confirm='Yes' and status='Amount Transferred'  and user='$tsname' and date between '$fromdate' and '$todate'   order by id desc";
	}
...
while($row	=	$result->fetch_assoc()){
    ...
    $krid=$row['id'];
    $ac=$row['ac_det'];
    $a="select store_code,ro_no,ro_date,falt_desc,tot_base,tot_ser,tot_gst,adv_amnt,adv_amnt1,adv_amnt2,gst_type,net,bal,invoice_date,invoice_no from add_tnqot where quet_num='$qot'";
	...
	$adamt="select sum(approve_amnt) as req_amnt from tnrequest_amnt where quet_num='$qot'and  status='Amount Transferred' and   confirm='Yes' and date between '$fromdate' and '$todate' " ;
	...
	$tyy="select sum(approve_amnt) as req_amnt from tnrequest_amnt where quet_num='$qot'   and status='' and  confirm='Yes' and ac_det='$ac' and date between '$fromdate' and '$todate' ";
...
$objPHPExcel->getActiveSheet()->SetCellValue('F'.$tc, 'Amount');
  ...
  if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts')){
    	...
    	$y2="select distinct ac_det from tnrequest_amnt where  user='$tsname' and  confirm='Yes' and status='' and date between '$fromdate' and '$todate' ";
	}
...
while($row10	=	$result10->fetch_assoc()){
    $acd=$row10['ac_det'];
	 ...
	 $r12="select sum(approve_amnt) as amt from tnrequest_amnt where ac_det='$acd' and confirm='Yes' and status='' and date between '$fromdate' and '$todate' ";
