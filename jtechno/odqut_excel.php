...
$objPHPExcel = PHPExcel_IOFactory::load("knqutoationtemplate.xlsx");
$id=$_GET['id'];
...
$objPHPExcel->getActiveSheet()->setCellValue('c2', $vendor_name4);	
$objPHPExcel->getActiveSheet()->setCellValue('c3', $kn_address);	
...
header("Content-Disposition: attachment;filename=".$quet_num." ".$city." ".BANG.".xlsx");
header('Cache-Control: max-age=0');
include_once('dbconnection/connection.php');
	 $aa="select  @acount:=@acount+1 sno,hsn,serv_code,desc1,brand,model,qty,uom,remarks,rate,base_amt,serv_amt,(base_amt+serv_amt) as total,gst_amnt  from (SELECT @acount:= 0) AS acount,add_odqot1 where  id1='$bid' ";
...
$finalrowvalue=$rowcount;    
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('I'.$rowCount, 'NON RC');
 else
 $objPHPExcel->getActiveSheet()->setCellValue('I'.$rowCount, 'RC');
 }
...
$aa="select count(base_amt) as counti  ,sum(base_amt) as base ,sum(serv_amt) as serv,sum(base_amt+serv_amt) as total,sum(gst_amnt) as gst from add_odqot1 where  id1='$bid' ";
...
$objPHPExcel->getActiveSheet()->setCellValue('K'.$ROWID,$t1['base']);
$objPHPExcel->getActiveSheet()->setCellValue('L'.$ROWID,$t1['serv']);
$objPHPExcel->getActiveSheet()->setCellValue('M'.$ROWID,$t1['total']);
$objPHPExcel->getActiveSheet()->setCellValue('N'.$ROWID,$t1['gst']);
$objPHPExcel->getActiveSheet()->setCellValue('N'.$ROWID1,$t1['gst']/2);
$objPHPExcel->getActiveSheet()->setCellValue('N'.$ROWID2,$t1['gst']/2);
$objPHPExcel->getActiveSheet()->setCellValue('N'.$ROWID3,0);
$objPHPExcel->getActiveSheet()->setCellValue('N'.$ROWID4,$t1['gst']);
$objPHPExcel->getActiveSheet()->setCellValue('N'.$ROWID5,round($t1['gst']+$t1['base']+$t1['serv']));
