...
<td>STATE:-TELANGANA1255<br/>STATE CODE:-36</td>
 ...
 $html .= ' <table style="font-size:8.5px;margin:15px;width:820px;" border="1"  cellpadding="0" cellspacing="0">
 <tr>
...
$sq=mysqli_query($link,"select distinct(sgst) from add_bill where service_no='$service_no' ");
 ...
 $aa="select * from add_bill where service_no='$service_no' and cgst='$sggst' and id1='$id1' ";
...
<td colspan="3" align="right" style="font-size:11px;"><b>Basic Amounts -'.$k.'</b></td><th>'.$tax_amnt.'</th></tr>
 ...
 $k++; } } 
	$html .='<tr><td colspan="3" rowspan="4"></td>
...
// different layout of odd and even pages
		...
		$html2pdf->Output('file_$id1.pdf', 'F');
		...
		$content = $html2pdf->Output('doc-'.$id1.'.pdf','F');
		
		
		
		header('Location:ap.php?id='.$bid.'&loc='.$loc.'&id1='.$id1);
