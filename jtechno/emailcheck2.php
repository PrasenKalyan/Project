...
function unique_name($service_no=''){
	    $query="select count(service_no) from add_gbill1 where `service_no`='$service_no'";
...
function unique_email($wcc_num=''){
	  $query1="select count(wcc_num) from add_gbill1 where `wcc_num`='$wcc_num'";
...
function unique_email1($wcc_rec_num=''){
	   $query2="select count(wcc_rec_num) from add_gbill1 where `wcc_rec_num`='$wcc_rec_num' ";
...
function unique_email2($req_ref=''){
	   $query2="select count(req_ref) from grefferences where `req_ref`='$req_ref' ";
	   $response2=mysql_query($query2);
	   $result2=mysql_fetch_array($response2);
	   if($result2[0]>0){
		  echo 'false';
