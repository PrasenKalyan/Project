...
if(isset($_POST['submit'])){
	...
	$tr=mysqli_fetch_array($t);
	 $empemail=$tr['emp_email'];
	  $a="INSERT INTO `add_odqot`(`store_code`, `inv_date`, `quet_num`, `tot_base`, `tot_ser`, `tot_gst`, `net`,  `ses`,falt_no,falt_date,falt_desc,status)
...
$sqq=mysqli_query($link,"select max(id) as ids from add_odqot ");
...
$ssq1=mysqli_query($link,"select * from add_odqot where id='$id'");
...
if($mark=="yes"){
    ...
    $kp=mysqli_query($link,"update add_odqot set status='Not Required',markfile='$iname1',wd='$wd' where id='$id' ");
...
`desc1`='".addslashes($pname)."', `hsn`='$hsn', `gst`='$gst', `uom`='$uom', `qty`='$qty', `gst_amnt`='$gst_amnt',
 ...
 $res=mysqli_query($link,$query) or die(mysqli_error($link));
  ...
  $query = "INSERT INTO add_odqot1 ( `desc1`, `hsn`, `gst`, `uom`, `qty`, `gst_amnt`, `rate`, `base_amt`,`date`,`id1`,item_id,serv_fee,cap,serv_amt,serv_code,serv_cap) 
	VALUES 
