						...
						$r=mysqli_fetch_array($sq);
						 $ac=$r['ac_det'];
											...
											$transfer_date=date('Y-m-d');
												...
												$sq=mysqli_query($link,"select * from  add_tnqot where where quet_num='$qt_no' ");
												$r2=mysqli_fetch_array($sq);
												$req_amnt_settled1=$r2['req_amnt_settled'];
												$req_amnt_settled=$req_amnt_settled1+$transfer;
											
											$uy="update add_tnqot set req_amnt_settled='$req_amnt_settled',status='Document Required' where quet_num='$qt_no'";
											$sq=mysqli_query($link,$uy) or die(mysqli_error($link));
																		
				
    									 $kk="update tnrequest_amnt set transfer='$transfer',transfer_date='$transfer_date',status='Amount Transferred' where id='$id'";
									//	exit;
