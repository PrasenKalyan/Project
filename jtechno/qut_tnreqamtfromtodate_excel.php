...
/*	if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts')){
	...
	if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts') or ($tsname=='sumanthpotluri')){
     $y="select quet_num,state,user,gstamt,gsttype from tnrequest_amnt where confirm='Yes' and status='Amount Transferred' and date >= '$fromdate' and date <='$todate' order by id desc";
	}else{
	$y="select  quet_num,state,user,gstamt,gsttype from tnrequest_amnt where confirm='Yes' and status='Amount Transferred' and user='$tsname' and date >='$fromdate' and date<='$todate' order by id desc";
	}
...
$objPHPExcel->getActiveSheet()->mergeCells('A1:U1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'TN REQUEST AMOUNT LIST');
...
while($row	=	$result->fetch_assoc()){
	...
	$tyy="select sum(req_amnt+gstamt) as rsamt from tnrequest_amnt where quet_num='$qot' and confirm='Yes'";
