	...
	if(in_array($_FILES["file"]["type"],$mimes)){
		...
		for($i=0;$i<$totalSheet;$i++){
			...
			foreach ($Reader as $Row)
	        {
				...
				$comp_date= date("Y-m-d", strtotime($comp_date1));
				 ...
				 $q="select * from refferences where location='$lname' and indus_id='$indus_id' and req_ref='$req_ref' ";
				...
				$sitetype1=$r['sitetype'];
					if( ($location1!= $lname) and ($sitetype1!= $sitetype) and ($indus_id1!= $indus_id ) and ($req_ref1!= $req_ref ) ){	
	 ...
	 //$res=mysql_query($ten_ins) or die(mysql_error());
				$link->query($query);
						...
						$m="update refferences set  site_name ='".addslashes($site_name)."' ,district='$district',  state='$state',seeking_id='$seeking_id', po_num='$po_num',allocation_date='$allocation_date2',comp_date='$comp_date',sitetype='$sitetype'  where indus_id='$indus_id1' and sitetype='$sitetype1' and location='$location1' and req_ref='$req_ref1' ";
