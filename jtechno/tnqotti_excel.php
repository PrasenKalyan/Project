...
$id=$_GET['id'];
$sq=mysqli_query($link,"select * from add_tnqot where quet_num='$id'");
...
$objPHPExcel->getActiveSheet()->setCellValue('I12',33);	
$objPHPExcel->getActiveSheet()->setCellValue('D13', $r['po_no']);
$objPHPExcel->getActiveSheet()->setCellValue('I11', TN);
$objPHPExcel->getActiveSheet()->setCellValue('I13', date('d-m-Y',strtotime($r['po_date'])));
$objPHPExcel->getActiveSheet()->setCellValue('C21', TN);
$objPHPExcel->getActiveSheet()->setCellValue('E21', 33);
...
$rowcounts=43;
include_once('dbconnection/connection.php');
 $ak="select @acount:=@acount+1 sno,hsn,gst,sum((base_amt+serv_amt)) as total,sum(gst_amnt) as gst_amnt from (SELECT @acount:= 0) AS acount,add_tnqot1 where id1='$bid'   group by hsn,gst order by sno";
