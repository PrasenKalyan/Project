...
$objPHPExcel->getActiveSheet()->setCellValue('B9','Karnataka');
$objPHPExcel->getActiveSheet()->setCellValue('B10',$format_type);
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header("Content-Disposition: attachment;filename=KN Summary sheet Miscellenious ".$quet_num." ".date('d-m-Y').".xlsx");
header('Cache-Control: max-age=0');
include_once('dbconnection/connection.php');
$aa="select  @acount:=@acount+1 sno,'$city','$store_code',falt_no,falt_date,falt_desc,quet_num,'',tot_base,tot_ser,tot_gst,(tot_base+tot_ser+tot_gst) as total  from (SELECT @acount:= 0) AS acount,add_knqot where id='$id' ";
