...
?>
										...
										//$adv=$adv+$r1['adv_amnt']+$r1['adv_amnt1']+$r1['adv_amnt2']-$rs1['req_amnt'];
										//$reqamt=$reqamt+$rs1['req_amnt'];
										// Ensure $bamt is initialized as an integer or float
$bamt = 0;

// Inside the loop where you're performing calculations
while ($rs1 = mysqli_fetch_array($t)) {
    // Check if $r1['bal'] is a numeric value
    if (is_numeric($r1['bal'])) {
        // Convert $r1['bal'] to a numeric type (float) and add it to $bamt
        $bamt += (float)$r1['bal'];
    } else {
        // Handle non-numeric values if needed
        // For example, skip adding them or log a warning
    }
}
