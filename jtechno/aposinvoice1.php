...
$q=mysqli_query($link,"select * from add_bill1 where id='$id'") or die(mysqli_error($link));
...
$sq=mysqli_query($link,"select distinct(sgst) from add_bill where service_no='$service_no' ");
 ...
 $aa="select * from add_bill where service_no='$service_no' and cgst='$sggst' and id1='$id'";
...
$html2pdf->setTestIsImage(false);
		...
		$content = $html2pdf->Output('apinvoice-'.$id.'.pdf','F');
		
		
		
		header('Location:ap.php?id='.$id.'&loc='.$loc.'&email='.$email.'&sno='.$service_no);
