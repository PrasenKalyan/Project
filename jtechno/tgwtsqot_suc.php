...
if(isset($_POST['submit'])){
	...
	$tr=mysqli_fetch_array($t);
	 $empemail=$tr['emp_email'];
	  $a="INSERT INTO `add_tgqot`(`store_code`, `inv_date`, `quet_num`, `tot_base`, `tot_ser`, `tot_gst`, `net`,  `ses`,falt_no,falt_date,falt_desc,status)
...
$ssq1=mysqli_query($link,"select * from add_tgqot where id='$id'");
...
if($mark=="yes"){
    ...
    $sq=mysqli_query($link,"update add_tgqot set status='Not Required',markfile='$iname1',wd='$wd' where id='$id' ") or die(mysqli_error($link));
...
if($ad!=$adv_amnt and $adv_amnt>=1){
	...
	$ssq2=mysqli_query($link,"insert into tgrequest_amnt(quet_num,req_amnt,req_date,user,state,ac_det,confirm,gstamt,gsttype)
	values('$qt_no','$adv_amnt','$adv_date','$ses','TG','$ac_det1','Pending','$gst_amount','$gst_type')");
}
if($ad1!=$adv_amnt1 and $adv_amnt1>=1){
	
	$ssq2=mysqli_query($link,"insert into tgrequest_amnt(quet_num,req_amnt,req_date,user,state,ac_det,confirm,gstamt,gsttype)
	values('$qt_no','$adv_amnt1','$adv_date1','$ses','TG','$ac_det2','Pending','$gst_amount1','$gst_type1')");
}
if($ad2!=$adv_amnt2 and $adv_amnt2>=1){
	
	$ssq2=mysqli_query($link,"insert into tgrequest_amnt(quet_num,req_amnt,req_date,user,state,ac_det,confirm,gstamt,gsttype)
	values('$qt_no','$adv_amnt2','$adv_date2','$ses','TG','$ac_det3','Pending','$gst_amount2','$gst_type2')");
}
