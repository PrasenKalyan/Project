...
if(isset($_POST['submit'])){
	...
	if(in_array($_FILES["file"]["type"],$mimes)){
		...
		for($i=0;$i<$totalSheet;$i++){
			...
			$html.="<td>".$i."</td>";
				...
				$html.="</tr>";
				 ...
				 $q="select * from mproducts where location='$lname' and category='$category' and item_code='$item_code' ";
	 ...
	 $query = "insert into mproducts(category,item_code,item_desc,primary_uom,qty,price_unit,hsn,sac,item_category,cgst,sgst,igst,location)
							values('".$category."','".$item_code."', '".addslashes($item_desc)."','".addslashes($primary_uom)."','".$qty."','".$price_unit."','".$hsn."','".$sac."','".addslashes($item_category)."','".$cgst."','".$sgst."','".$igst."','".$lname."')";     
	 ...
	 //$res=mysql_query($ten_ins) or die(mysql_error());
				$link->query($query);
					...
					$m="update mproducts set  item_desc ='".addslashes($item_desc)."' ,primary_uom='$primary_uom', qty='$qty',price_unit='$price_unit', hsn='$hsn',sac='$sac',item_category='$item_category', cgst='$cgst',sgst='$sgst', igst='$igst'    where category='$category1' and item_code='$item_code1' and location='$location1' ";
