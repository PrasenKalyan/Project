...
$falt_desc=$_POST['falt_desc'];
	...
	$tr=mysqli_fetch_array($t);
	 $empemail=$tr['emp_email'];
	  $a="INSERT INTO `add_qot`(`store_code`, `inv_date`, `quet_num`, `tot_base`, `tot_ser`, `tot_gst`, `net`,  `ses`,falt_no,falt_date,falt_desc)
	 VALUES ('$store_code','$inv_date','$qt_no','$tot','$tot_serv','$tot_gst','$net','$ses','$falt_no','$falt_date','$falt_desc')";
...
$query = "INSERT INTO add_qot1 ( `desc1`, `hsn`, `gst`, `uom`, `qty`, `gst_amnt`, `rate`, `base_amt`,`date`,`id1`,item_id,serv_fee,cap,serv_amt,serv_code) 
	VALUES 
 ('".addslashes($pname)."','$hsn','$gst','$uom','$qty','$gst_amnt','$price','$amnt','$date','$sno','$id','$serv_amt','$cap','$serv_amnt','$serv_code')";
...
$ssq1=mysqli_query($link,"select * from add_qot2 where id='$id'");
...
if($ad!=$adv_amnt and $adv_amnt>=1){
	...
	values('$qt_no','$adv_amnt','$adv_date','$ses','TS','$ac_det1')");
}
if($ad1!=$adv_amnt1 and $adv_amnt1>=1){
	
	$ssq2=mysqli_query($link,"insert into request_amnt(quet_num,req_amnt,req_date,user,state,ac_det)
	values('$qt_no','$adv_amnt1','$adv_date1','$ses','TS','$ac_det2')");
}
if($ad2!=$adv_amnt2 and $adv_amnt2>=1){
	
	$ssq2=mysqli_query($link,"insert into request_amnt(quet_num,req_amnt,req_date,user,state,ac_det)
	values('$qt_no','$adv_amnt2','$adv_date2','$ses','TS','$ac_det3')");
}

		   $v="update add_qot2 set
		 
		 `store_code`='$store_code' ,`tot_base`='$tot', `tot_ser`='$tot_serv', `tot_gst`='$tot_gst', `net`='$net', 
		 falt_no='$falt_no',falt_date='$falt_date',falt_desc='$falt_desc',ro_no='$ro_no',ro_date='$ro_date',
		 po_no='$po_no',po_date='$po_date',po_type='$po_type',type_of_work='$type_of_work',sub_type='$sub_type',status='$status',
		 ...
		 ,img1='$iname1',img2='$iname3',img3='$iname5',bal='$bal',gst_type='$gst_type'
