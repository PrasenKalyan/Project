...
header("Content-disposition: attachment; filename=Minvoice_summary".date('d-m-Y').".xls");
...
<center><h2><u>Mahatasthra Summery</u></h2></center>
<div >
    <table class="left" border="1">
      ...
      <tr>
      <?php $sq=mysqli_query($link,"select * from add_mbill1 where `date` between '$from' and '$to'");
	     $i=1;
	  while($r=mysqli_fetch_array($sq))
	  {
		$rf=$r['req_ref'];
		$t=mysqli_query($link,"select * from mrefferences where req_ref='$rf'");
	 ...
	 <td><?php echo $r['po_date'];?></td>
	  <td><?php echo $r['opo_no'];?></td>
	 <td><?php echo $r['opo_date'];?></td>
