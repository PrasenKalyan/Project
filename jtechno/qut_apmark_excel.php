...
$objPHPExcel->getActiveSheet()->mergeCells('A1:K1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', $state.' QUOTATION  MARK NOT REQUIRED LIST');
...
while($row	=	$result->fetch_assoc()){
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('J'.$rowCount, mb_strtoupper($row['docr_status'],'UTF-8'));
		$objPHPExcel->getActiveSheet()->SetCellValue('K'.$rowCount, mb_strtoupper($row['user'],'UTF-8'));
	$rowCount++;
$i++; }
...
header('Content-Disposition: attachment;filename="'.$state.'-marklist.xlsx"'); //tell browser what's the file name
