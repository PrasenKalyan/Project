...
$id=$_POST['id'];
   ...
   if($id1!='' and $id2!=''){
	  $query = "update add_bill set `service_no`='$service_no', `item_code`='$item_code',item_desc='".$item_desc."', `price`='$price',uom='$uom',
 ...
 $res=mysqli_query($link,$query) or trigger_error("Update failed: " . mysqli_error($link));
	  ...
	  // if($id1!='' and $id=''){
	 echo  $query = "INSERT INTO add_bill ( `service_no`, `item_code`, `price`, `qty`, `tax_amnt`, `gst_amnt`, `sgst`, `cgst`,`date`,`temp_type`,`uom`,`item_desc`,`sgstamount`,`cgstamount`,`sno`,`id1`,`hsnno`,`sacno`) 
	VALUES 
